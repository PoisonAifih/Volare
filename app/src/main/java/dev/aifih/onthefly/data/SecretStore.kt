...
/**
 * Stores one secret encrypted with an AES-GCM key that lives in the Android Keystore, so the
 * raw value never touches disk in plaintext and cannot be read by adb backup or another app.
 *
 ...
 *
 * Each secret gets its own [keyAlias]; revoking one must not make the others unreadable.
 */
class SecretStore(
    context: Context,
